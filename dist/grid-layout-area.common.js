...
/*: Layout*/
{
  ...
  for (var _i3 = 0, len = sorted.length; _i3 < len; _i3++) {
    ...
    l.moved = false;
  } // console.log(2,JSON.stringify(layout),JSON.stringify(compareWith),JSON.stringify(out))
...
/* harmony default export */ var GridItemvue_type_script_lang_js_ = ({
  ...
  created: function created() {
    ...
    self.setFixarea = function (fix) {
      self.isFixarea = fix;
    };

    this.eventBus.$on('updateWidth', self.updateWidthHandler);
    this.eventBus.$on('compact', self.compactHandler);
    this.eventBus.$on('setFixarea', self.setFixarea);
  ...
  mounted: function mounted() {
    ...
    this.$nextTick(function () {
      _this2.cols = _this2.$parent.colNum;
      _this2.rowHeight = _this2.$parent.height;
      _this2.containerWidth = _this2.$parent.width !== null ? _this2.$parent.width : 100;
      _this2.margin = _this2.$parent.margin !== undefined ? _this2.$parent.margin : [10, 10];
      _this2.maxRows = _this2.$parent.maxRows;

      if (_this2.isDraggable === null) {
        _this2.draggable = _this2.$parent.isDraggable;
      } else {
        _this2.draggable = _this2.isDraggable;
      }

      if (_this2.isResizable === null) {
        _this2.resizable = _this2.$parent.isResizable;
      } else {
        _this2.resizable = _this2.isResizable;
      }

      if (_this2.isFixarea === null) {
        _this2.isFixarea = _this2.$parent.isFixarea;
      } else {
        _this2.isFixarea = _this2.isFixarea;
      }

      _this2.useCssTransforms = _this2.$parent.useCssTransforms;
        ...
        case "resizestart":
          {
            ...
            this.oldPostion = [this.x, this.y, this.w, this.h];
        ...
        case "resizemove":
          {
            ...
            if (this.isFixarea && newSize.height / this.rowHeight + this.y > this.$parent.rowHeight) {
              break;
        ...
        case "dragend":
          {
            ...
            this.eventBus.$emit('judgeFixArea', function (hasMove) {
              if (hasMove) {
                ...
                var pos = _this4.calcPosition.apply(_this4, _toConsumableArray(_this4.oldPostion));
        ...
        case "dragmove":
          {
            ...
            newPosition.top = this.dragging.top + coreEvent.deltaY; //cj 限制区域范围

            var pose = this.calcXY(newPosition.top, newPosition.left);

            if (this.isFixarea && this.h + pose.y > this.$parent.rowHeight) {
              break;
...
/* harmony default export */ var GridLayoutvue_type_script_lang_js_ = ({
  ...
  created: function created() {
    ...
    self.eventBus.$on('judgeFixArea', self.judgeFixArea);
  },
  beforeDestroy: function beforeDestroy() {
    //Remove listeners
    this.eventBus.$off('resizeEvent', this.resizeEventHandler);
    this.eventBus.$off('dragEvent', this.dragEventHandler);
    self.eventBus.$off('judgeFixArea', self.judgeFixArea);
    removeWindowEventListener("resize", this.onWindowResize);
  },
  mounted: function mounted() {
    if (this.isFixarea && this.$el.parentElement.scrollHeight > 0) {
      this.height = this.$el.parentElement.scrollHeight / this.rowHeight;
    } else {
  ...
  watch: {
    ...
    isFixarea: function isFixarea() {
      this.eventBus.$emit("setFixarea", this.isFixarea);
    },
    isResizable: function isResizable() {
  ...
  methods: {
    ...
    containerHeight: function containerHeight() {
      ...
      return bottom(this.layout) * (this.height + this.margin[1]) + this.margin[1] + 'px';
    },
    ...
    //判断在固定区域范围的时候，是否超出了范围
    judgeFixArea: function judgeFixArea(callback) {
      var hasMove = true;

      if (this.isFixarea) {
        for (var i in this.layout) {
          if (this.layout[i].y + this.layout[i].h > this.rowHeight) {
            hasMove = false;
            break;
          }
        }
...
var GridLayout_component = normalizeComponent(
  ...
  GridLayoutvue_type_template_id_7b1a1ed1_render,
  GridLayoutvue_type_template_id_7b1a1ed1_staticRenderFns,
  false,
  null,
  null,
  null
...
/******/ })["default"];
//# sourceMappingURL=grid-layout-area.common.js.map